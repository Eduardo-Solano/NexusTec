        ...
        <div class="flex justify-between h-20">
            ...
            <div class="flex">
                ...
                <div class="hidden space-x-8 xl:-my-px xl:ms-10 xl:flex">
                    ...
                    <x-nav-link :href="route('events.index')" :active="request()->routeIs('events.*')" class="text-gray-300 hover:text-white transition-colors duration-300">
                        {{ __('Eventos') }}
                    </x-nav-link>

                    @can('teams.edit')
                        <x-nav-link :href="route('teams.index')" :active="request()->routeIs('teams.*')" class="text-gray-300 hover:text-white transition-colors duration-300">
                            {{ __('Equipos') }}
                        </x-nav-link>
                    @endcan

                    @can('projects.edit')
                        <x-nav-link :href="route('projects.index')" :active="request()->routeIs('projects.*')" class="text-gray-300 hover:text-white transition-colors duration-300">
                            {{ __('Proyectos') }}
                        </x-nav-link>
                    @endcan

                    @can('criteria.view')
                        <x-nav-link :href="route('criteria.index')" :active="request()->routeIs('criteria.*')" class="text-gray-300 hover:text-white transition-colors duration-300">
                            {{ __('Criterios') }}
                        </x-nav-link>
                    @endcan
                    ...
                    @role('admin')
                        <x-nav-link :href="route('activity-logs.index')" :active="request()->routeIs('activity-logs.*')" class="text-gray-300 hover:text-white transition-colors duration-300">
                            {{ __('Actividad') }}
                        </x-nav-link>
                ...
                @auth
                    <div class="relative me-3 ms-6 group">
                        ...
                        <button @click="notificationsOpen = !notificationsOpen"
                            ...
                            <svg class="h-6 w-6 transform group-hover:scale-110 transition-transform duration-200" fill="none" viewBox="0 0 24 24">
                        ...
                        <div x-show="notificationsOpen" @click.away="notificationsOpen = false" x-transition
                            ...
                            <!-- Header -->
                            <div class="px-5 py-4 bg-white/5 border-b border-white/10 flex justify-between items-center backdrop-blur-md">
                                <h3 class="text-sm font-bold text-white flex items-center gap-2">
                                    <svg class="w-4 h-4 text-blue-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.4-1.4A2 2 0 0118 14.2V11a6 6 0 00-4-5.7V4a2 2 0 10-4 0v1.3A6 6 0 006 11v3.2a2 2 0 01-.6 1.4L4 17h5m6 0v1a3 3 0 11-6 0v-1" /></svg>
                                    Notificaciones
                                </h3>
                                @if ($unreadNotifications->count() > 0)
                                    <form action="{{ route('notifications.markAllAsRead') }}" method="POST">
                                        @csrf
                                        <button class="text-xs font-medium text-blue-400 hover:text-blue-300 hover:underline transition-colors">
                                            Marcar leídas
                                        </button>
                                    </form>
                                @endif
                            ...
                            <div class="max-h-[28rem] overflow-y-auto custom-scrollbar">
                                ...
                                @foreach ($unreadNotifications as $notification)
                                    @if (($notification->data['type'] ?? null) === 'join_request')
                                        <div class="px-5 py-4 hover:bg-white/5 border-b border-white/5 transition-colors group">
                                            <div class="flex items-start gap-4">
                                                <div class="p-2.5 bg-indigo-500/20 rounded-xl group-hover:bg-indigo-500/30 transition-colors">
                                                    <svg class="w-5 h-5 text-indigo-400" viewBox="0 0 24 24" fill="none">
                                                        <path d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM4 19a4 4 0 014-4h8a4 4 0 014 4" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                                                    </svg>
                                                </div>
                                                <div class="flex-1">
                                                    ...
                                                    <p class="text-xs text-indigo-400/80 mt-1 font-medium">Equipo: {{ $notification->data['team_name'] ?? 'Sin nombre' }}</p>
                                                    <p class="text-[10px] text-gray-500 mt-2">{{ $notification->created_at->diffForHumans() }}</p>
                                                    <div class="flex gap-2 mt-3">
                                                        <form method="POST" action="{{ route('teams.accept', ['team' => $notification->data['team_id'], 'user' => $notification->data['user_id']]) }}">
                                                            @csrf <input type="hidden" name="notification" value="{{ $notification->id }}">
                                                            <button type="submit" class="px-4 py-1.5 text-xs font-bold rounded-lg bg-green-500/20 text-green-400 border border-green-500/20 hover:bg-green-500/30 transition-colors">Aceptar</button>
                                                        </form>
                                                        <form method="POST" action="{{ route('teams.reject', ['team' => $notification->data['team_id'], 'user' => $notification->data['user_id']]) }}">
                                                            @csrf <input type="hidden" name="notification" value="{{ $notification->id }}">
                                                            <button type="submit" class="px-4 py-1.5 text-xs font-bold rounded-lg bg-red-500/20 text-red-400 border border-red-500/20 hover:bg-red-500/30 transition-colors">Rechazar</button>
                                                        </form>
                                                    </div>
                                                </div>
                                            </div>
                                        ...
                                        <div class="px-5 py-4 hover:bg-white/5 border-b border-white/5 transition-colors">
                                            <div class="flex items-start gap-4">
                                                <div class="p-2.5 bg-blue-500/20 rounded-xl">
                                                    <svg class="w-5 h-5 text-blue-400" fill="none" viewBox="0 0 24 24"><path stroke="currentColor" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM4 19a4 4 0 014-4h8a4 4 0 014 4" /></svg>
                                                </div>
                                                <div class="flex-1">
                                                    <p class="text-sm font-semibold text-gray-200">{{ $notification->data['message'] ?? 'Has sido invitado a un equipo' }}</p>
                                                    <p class="text-xs text-gray-500 mt-1">Equipo: <span class="text-gray-300 font-medium">{{ $notification->data['team_name'] ?? 'Sin nombre' }}</span></p>
                                                    <div class="flex gap-2 mt-3">
                                                        <form action="{{ $notification->data['accept_url'] ?? '#' }}" method="POST">
                                                            @csrf <button type="submit" class="px-4 py-1.5 text-xs font-bold bg-green-500/20 text-green-400 rounded-lg hover:bg-green-500/30 border border-green-500/20 transition">Aceptar</button>
                                                        </form>
                                                        <form action="{{ $notification->data['reject_url'] ?? '#' }}" method="POST">
                                                            @csrf <button type="submit" class="px-4 py-1.5 text-xs font-bold bg-red-500/20 text-red-400 rounded-lg hover:bg-red-500/30 border border-red-500/20 transition">Rechazar</button>
                                                        </form>
                                                    </div>
                                                </div>
                                            </div>
                                ...
                                @foreach ($unreadNotifications as $notification)
                                    @if (($notification->data['type'] ?? null) === 'team_join_response')
                                        ...
                                        <a href="{{ $notification->data['team_url'] ?? '#' }}" onclick="fetch('{{ route('notifications.markAsRead', $notification->id) }}', { method: 'POST', headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}' } })" class="block px-5 py-4 hover:bg-white/5 border-b border-white/5 transition-colors">
                                            <div class="flex items-start gap-4">
                                                <div class="p-2.5 rounded-xl {{ $accepted ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400' }}">
                                                    <svg class="w-5 h-5" viewBox="0 0 24 24" fill="none"><path d="{{ $accepted ? 'M5 13l4 4L19 7' : 'M6 6l12 12M6 18L18 6' }}" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" /></svg>
                                                </div>
                                                <div class="flex-1">
                                                    <p class="text-sm font-semibold text-gray-200">{{ $notification->data['message'] ?? 'Actualización sobre tu solicitud.' }}</p>
                                ...
                                @if ($pendingAdvisories->count() > 0)
                                    <div class="px-5 py-2 bg-indigo-500/10 border-b border-white/5"><p class="text-xs font-bold text-indigo-400 uppercase tracking-wider">Solicitudes de Asesoría</p></div>
                                        ...
                                        <div class="px-5 py-4 hover:bg-white/5 border-b border-white/5">
                                            <div class="flex text-gray-300 justify-between items-center">
                                                ...
                                                <div class="flex gap-2">
                                                    <form action="{{ route('teams.advisor.response', ['team' => $advisory, 'status' => 'accepted']) }}" method="POST">@csrf @method('PATCH')<button class="text-green-400 hover:text-green-300 font-bold text-xs">✓</button></form>
                                                    <form action="{{ route('teams.advisor.response', ['team' => $advisory, 'status' => 'rejected']) }}" method="POST">@csrf @method('PATCH')<button class="text-red-400 hover:text-red-300 font-bold text-xs">✕</button></form>
                                                </div>
                                            </div>
                                        </div>
                                    @endforeach
                                ...
                                @if ($pendingEvaluations->count() > 0)
                                    <div class="px-5 py-2 bg-amber-500/10 border-b border-white/5"><p class="text-xs font-bold text-amber-500 uppercase tracking-wider">Por Evaluar</p></div>
                                    @foreach ($pendingEvaluations as $project)
                                        <div class="px-5 py-3 hover:bg-white/5 border-b border-white/5 flex justify-between items-center">
                                            <div class="text-sm text-gray-300">
                                                <p class="font-bold">{{ $project->name }}</p>
                                                <p class="text-xs text-gray-500">{{ $project->team->name }}</p>
                                            </div>
                                            <a href="{{ route('projects.show', $project) }}" class="px-3 py-1 bg-amber-500/20 text-amber-400 text-xs rounded hover:bg-amber-500/30">Ver</a>
                                        </div>
                                    @endforeach
                                @endif
                                ...
                                @foreach ($unreadNotifications as $notification)
                                    ...
                                    @if ($type !== 'join_request' && $type !== 'team_invitation' && $type !== 'team_join_response')
                                        <a href="{{ route('public.event-winners', $notification->data['event_id'] ?? 0) }}" onclick="fetch('{{ route('notifications.markAsRead', $notification->id) }}', { method: 'POST', headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}' } })" class="block px-5 py-4 hover:bg-white/5 border-b border-white/5 transition-colors">
                                            <div class="flex items-center gap-4">
                                                <div class="text-2xl animate-bounce">🏆</div>
                                                <div class="flex-1">
                                                    <p class="text-sm font-bold text-yellow-100">{{ $notification->data['award_category'] ?? 'Premio' }}</p>
                                                    <p class="text-xs text-yellow-500/70">{{ $notification->data['team_name'] ?? 'Tu equipo' }}</p>
                                                </div>
                                            </div>
                                        </a>
                                    @endif
                                ...
                                @if ($totalNotifications === 0)
                                    <div class="px-4 py-12 text-center">
                                        <div class="inline-flex p-4 bg-white/5 rounded-full mb-4">
                                            <svg class="w-8 h-8 text-gray-500" fill="none" viewBox="0 0 24 24"><path stroke="currentColor" stroke-width="2" d="M15 17h5l-1.4-1.4A2 2 0 0118 14.2V11a6 6 0 00-4-5.7V4a2 2 0 10-4 0v1.3A6 6 0 006 11v3.2a2 2 0 01-.6 1.4L4 17h5m6 0v1a3 3 0 11-6 0v-1" /></svg>
                                        </div>
                ...
                <div class="hidden xl:flex xl:items-center xl:ms-6">
                        ...
                        <x-dropdown align="right" width="48">
                            ...
                            <x-slot name="content">
                                ...
                                <form method="POST" action="{{ route('logout') }}">
                                    @csrf
                                    <x-dropdown-link :href="route('logout')" onclick="event.preventDefault(); this.closest('form').submit();" class="text-red-400 hover:text-red-300 hover:bg-red-500/10">
                                        Cerrar Sesión
                                    </x-dropdown-link>
                                </form>
                            </x-slot>
                    ...
                    @else
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <a href="{{ route('register') }}" class="text-sm font-bold px-5 py-2 bg-gradient-to-r from-cyan-500 to-blue-500 hover:from-cyan-400 hover:to-blue-400 text-white rounded-lg shadow-lg shadow-cyan-500/20 transition-all hover:-translate-y-0.5">Registrarse</a>
                        </div>
                    @endauth
                </div>
        ...
        <div x-show="open" class="relative z-50">
            ...
            <div x-show="open" x-transition:enter="transition ease-out duration-300 transform"
                ...
                <div class="flex items-center justify-between px-6 h-20 bg-gradient-to-r from-blue-900/20 to-purple-900/20 border-b border-white/10">
                    <span class="text-xl font-black text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-purple-400">NEXUS</span>
                    <button @click="open = false" class="text-gray-400 hover:text-white p-2 hover:bg-white/10 rounded-lg transition-colors">
