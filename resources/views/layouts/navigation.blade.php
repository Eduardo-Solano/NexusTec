<nav x-data="{ open: false, notificationsOpen: false }"
    class="sticky top-0 z-40 bg-white/80 dark:bg-gray-800/80 backdrop-blur-md border-b border-gray-200/50 dark:border-gray-700/50 shadow-sm">
                    ...
                    @if (auth()->user()?->can('students.view') || auth()->user()?->can('staff.view') || auth()->user()?->can('judges.view'))
                        <div class="hidden xl:flex xl:items-center xl:ms-2">
                            <x-dropdown align="left" width="48">
                                <x-slot name="trigger">
                                    <button
                                        class="inline-flex items-center px-3 py-2 text-sm border border-transparent rounded-md text-gray-500 dark:text-gray-400 bg-white dark:bg-gray-800 hover:text-gray-700 dark:hover:text-gray-300 transition ease-in-out duration-150">
                                        <span>Usuarios</span>
                ...
                @auth
                    <div class="relative me-3 ms-6 group">
                        ...
                        <button @click="notificationsOpen = !notificationsOpen"
                            class="relative text-gray-500 hover:text-gray-700 dark:text-gray-300 focus:outline-none transition-all duration-200">
                            ...
                            @if ($totalNotifications > 0)
                                <span
                                    class="absolute -top-1 -right-1 h-5 w-5 flex items-center justify-center bg-red-600 text-white rounded-full text-[10px] font-bold group-hover:opacity-70 transition-opacity duration-200">
                            ...
                            <div class="px-4 py-3 bg-white dark:bg-gray-800 flex justify-between">
                                ...
                                @if ($unreadNotifications->count() > 0)
                                    <form action="{{ route('notifications.markAllAsRead') }}" method="POST">
                                        @csrf
                                        <button class="text-xs text-blue-600 dark:text-blue-400 hover:underline">Marcar
                                            leídas</button>
                                    </form>
                                @endif
                            </div>
                                                ...
                                                <div class="flex-1">
                                                    ...
                                                    <div class="flex gap-2 mt-2">
                                                        <form
                                                            ...
                                                            @csrf @method('PATCH')
                                                            <button
                                                                class="px-2.5 py-1 text-xs bg-green-600 text-white rounded-lg">Aceptar</button>
                                                        </form>
                                                        <form
                                                            ...
                                                            @csrf @method('PATCH')
                                                            <button
                                                                class="px-2.5 py-1 text-xs bg-red-600 text-white rounded-lg">Rechazar</button>
                                                        </form>
                                                    </div>
                                                </div>
                                            </div>
                                        ...
                                        <div
                                            ...
                                            <div class="flex items-center gap-3">
                                                ...
                                                <a href="{{ route('projects.show', $project) }}"
                                                    class="px-3 py-1.5 text-xs font-bold bg-amber-600 text-white rounded-lg">Ver</a>
                                            </div>
                                        </div>
                                    @endforeach
                                @endif
                ...
                <div class="hidden xl:flex xl:items-center xl:ms-6">
                    @auth
                        <x-dropdown align="right" width="48">
                            <x-slot name="trigger">
                                <button
                                    class="inline-flex items-center px-3 py-2 rounded-md text-sm text-gray-500 dark:text-gray-400 bg-white dark:bg-gray-800 hover:text-gray-700 dark:hover:text-gray-300 focus:outline-none transition ease-in-out duration-150">
                    ...
                    @else
                        <div class="flex items-center gap-4">
                            ...
                            <a href="{{ route('register') }}"
                                class="text-sm font-medium px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition">
                                Registrarse
                            </a>
                        </div>
                    @endauth
    ...
    <template x-teleport="body">
        <div x-show="open" class="relative z-50">
            ...
            <div x-show="open" x-transition:enter="transition-opacity ease-linear duration-300"
                ...
                class="fixed inset-0 bg-gray-900/80 backdrop-blur-sm" @click="open = false">
            </div>
