                            ...
                            @foreach($data['pending_advisories'] as $reqProject)
                                <div class="bg-gray-900 p-4 rounded-xl flex items-center justify-between border border-gray-700">
                                    ...
                                    <div class="flex gap-2">
                                        <form action="{{ route('projects.advisor.response', ['project' => $reqProject, 'status' => 'accepted']) }}" method="POST">
                                            @csrf @method('PATCH')
                                            <button type="submit" class="px-4 py-2 bg-green-600 hover:bg-green-500 text-white text-xs font-bold rounded-lg transition">Aceptar</button>
                                        </form>
                                        <form action="{{ route('projects.advisor.response', ['project' => $reqProject, 'status' => 'rejected']) }}" method="POST">
                                            @csrf @method('PATCH')
                                            <button type="submit" class="px-4 py-2 bg-red-600 hover:bg-red-500 text-white text-xs font-bold rounded-lg transition">Rechazar</button>
                                        </form>
                                    </div>
                                </div>
                            @endforeach
                    ...
                    <div class="mb-8">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            @foreach($myAdvisories as $project)
                                <div class="bg-gray-800 border border-gray-700 rounded-xl p-5 shadow-lg hover:border-green-500/50 transition group">
                                    ...
                                    <h4 class="text-lg font-bold text-white mb-1 group-hover:text-green-400 transition">{{ $project->name }}</h4>
                                    ...
                                    <div class="pt-4 border-t border-gray-700 flex justify-between items-center">
                                        <a href="{{ route('projects.show', $project) }}" class="text-sm font-bold text-white hover:text-green-400 transition">Ver Proyecto &rarr;</a>
                        ...
                        @role('admin')
                            <a href="{{ route('staff.index') }}" class="group bg-gradient-to-br from-indigo-900 to-blue-900 p-6 rounded-2xl border border-indigo-500/30 shadow-lg hover:shadow-indigo-500/20 transition transform hover:-translate-y-1 relative overflow-hidden">
                                ...
                                <div class="flex items-center justify-between mb-4">
                                    ...
                                    <span class="p-2 bg-indigo-500/20 text-white rounded-lg group-hover:bg-indigo-500 transition"><svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"/></svg></span>
                                </div>
                                <div class="flex items-end justify-between">
                                    <p class="text-3xl font-black text-white">{{ \App\Models\User::role(['staff', 'advisor'])->count() }}</p>
                                    <span class="text-xs text-indigo-300 font-medium group-hover:text-white transition">Gestionar &rarr;</span>
                                </div>
                            </a>
                        @endrole
                    </div>
                            ...
                            <ul class="space-y-4">
                                @foreach($data['recent_teams'] as $team)
                                    <li class="flex justify-between items-center text-sm">
                                        ...
                                        <a href="{{ route('teams.show', $team) }}" class="text-ito-orange hover:text-white font-bold text-xs bg-orange-500/10 px-3 py-1 rounded transition hover:bg-orange-500">Ver</a>
                                    </li>
                                @endforeach
                            </ul>
                        </div>
                    ...
                    <div class="lg:col-span-2 space-y-6">
                        ...
                        @if(isset($data['my_team']))
                            <div class="bg-gray-800 border border-gray-700 rounded-2xl p-6 shadow-lg relative overflow-hidden group">
                                ...
                                <div class="relative z-10">
                                    ...
                                    <div class="bg-gray-900/50 p-4 rounded-xl border border-gray-700 mb-6 flex items-center gap-4">
                                        <div class="w-12 h-12 rounded-lg bg-gray-800 flex items-center justify-center text-2xl border border-gray-600">🛡️</div>
                                        <div>
                                    ...
                                    <div class="grid grid-cols-2 gap-4">
                                        <a href="{{ route('events.show', $data['my_team']->event_id) }}" class="flex items-center justify-center py-3 bg-white text-gray-900 font-bold rounded-xl hover:bg-gray-200 transition">Ver Detalles</a>
                                        @if($data['my_team']->project)
                                            <a href="{{ route('projects.show', $data['my_team']->project) }}" class="flex items-center justify-center py-3 bg-gray-700 text-white font-bold rounded-xl hover:bg-gray-600 transition border border-gray-600">Ver Proyecto</a>
                                        @else
                                            <a href="{{ route('projects.create', ['team_id' => $data['my_team']->id]) }}" class="flex items-center justify-center py-3 bg-purple-600 text-white font-bold rounded-xl hover:bg-purple-500 transition shadow-lg shadow-purple-500/20">Entregar Proyecto</a>
                                        @endif
                                    </div>
                                </div>
                            </div>
                        @else
                            <div class="bg-gray-800 border-2 border-dashed border-gray-700 rounded-2xl p-10 text-center">
                                ...
                                <a href="{{ route('events.index') }}" class="inline-block px-6 py-3 bg-ito-orange hover:bg-orange-600 text-white font-bold rounded-xl transition">Ver Eventos</a>
                            </div>
                        @endif
                    </div>
                            ...
                            <ul class="space-y-4">
                                @forelse($data['upcoming_events'] ?? [] as $evt)
                                    <li class="flex items-center gap-3">
                                        ...
                                        <a href="{{ route('events.show', $evt) }}" class="p-2 text-gray-400 hover:text-white hover:bg-gray-700 rounded-lg transition">&rarr;</a>
                                    </li>
                                @empty
