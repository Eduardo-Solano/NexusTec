    ...
    <div class="min-h-screen bg-[#0B1120] py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <nav class="flex items-center text-sm font-medium text-gray-400 mb-8">
                <a href="{{ route('judges.index') }}" class="hover:text-white transition flex items-center group">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                {{-- Panel Izquierdo: Información del Juez --}}
                <div class="lg:col-span-1">
                    <div class="bg-gray-800 border border-gray-700 rounded-2xl p-8 shadow-xl text-center relative overflow-hidden h-full">
                        <div class="absolute top-0 left-0 w-full h-32 bg-gradient-to-b from-purple-900/50 to-transparent"></div>
                        ...
                        <div class="relative z-10">
                            <div class="w-28 h-28 mx-auto bg-gradient-to-br from-gray-800 to-black rounded-full flex items-center justify-center border-4 border-gray-700 mb-4 shadow-2xl relative group">
                                <div class="absolute inset-0 bg-purple-500/20 rounded-full blur-xl opacity-0 group-hover:opacity-100 transition duration-500"></div>
                            ...
                            <h2 class="text-xl font-bold text-white mb-1">{{ $judge->name }}</h2>
                            <div class="inline-flex items-center px-3 py-1 rounded-full bg-purple-900/30 border border-purple-500/30 text-purple-300 text-xs font-bold mb-2">
                                {{ $judge->judgeProfile->specialty->name ?? 'Sin especialidad' }}
                            </div>
                            <p class="text-gray-400 text-sm mb-6">{{ $judge->judgeProfile->company ?? 'Sin empresa' }}</p>
                            ...
                            <div class="border-t border-gray-700 pt-6 grid grid-cols-1 gap-4 text-left">
                                <div class="bg-gray-900/50 p-3 rounded-lg border border-gray-700">
                                    ...
                                    <p class="text-sm text-white font-mono font-bold">#{{ str_pad($judge->id, 4, '0', STR_PAD_LEFT) }}</p>
                                </div>
                                <div class="bg-gray-900/50 p-3 rounded-lg border border-gray-700">
                                    <p class="text-[10px] uppercase text-gray-500 font-bold tracking-wider mb-1">Estado</p>
                                    <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-bold {{ $judge->is_active ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400' }}">
                                        <span class="w-1.5 h-1.5 rounded-full mr-1.5 {{ $judge->is_active ? 'bg-green-400' : 'bg-red-400' }}"></span>
                                        {{ $judge->is_active ? 'Activo' : 'Inactivo' }}
                                    </span>
                                </div>
                                <div class="bg-gray-900/50 p-3 rounded-lg border border-gray-700">
                                    <p class="text-[10px] uppercase text-gray-500 font-bold tracking-wider mb-1">Fecha de Alta</p>
                                    <p class="text-sm text-white font-mono">{{ $judge->created_at->format('d/m/Y') }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-gray-800 border border-gray-700 rounded-2xl shadow-xl overflow-hidden">
                        ...
                        <div class="border-b border-gray-700 px-8 py-6 bg-gray-800 flex justify-between items-center">
                            <div>
                                ...
                                <p class="text-sm text-gray-400">Actualiza los datos del juez externo.</p>
                            </div>
                            <span class="w-2 h-2 rounded-full {{ $judge->is_active ? 'bg-green-500 shadow-[0_0_10px_rgba(34,197,94,0.6)]' : 'bg-red-500 shadow-[0_0_10px_rgba(239,68,68,0.6)]' }}"></span>
                        </div>
                        ...
                        <form action="{{ route('judges.update', $judge) }}" method="POST">
                            ...
                            <div class="p-8 space-y-8">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div class="space-y-2">
                                        <x-input-label for="specialty_id" :value="__('Especialidad')" class="text-white font-bold text-xs uppercase tracking-wide" />
                                        <select name="specialty_id" id="specialty_id" class="block w-full bg-gray-900 border-gray-600 text-white rounded-lg shadow-sm focus:border-purple-500 focus:ring-purple-500 h-11 text-sm @error('specialty_id') border-red-500 @enderror">
                                            <option value="">-- Selecciona --</option>
                                            @foreach ($specialties as $specialty)
                                                <option value="{{ $specialty->id }}" @if(old('specialty_id', $judge->judgeProfile->specialty_id ?? '') == $specialty->id) selected @endif>
                                                    {{ $specialty->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                        @error('specialty_id')<p class="text-red-400 text-sm mt-1">{{ $message }}</p>@enderror
                                    </div>
                                </div>
                                ...
                                <div class="bg-gray-900/50 border border-gray-700 rounded-xl p-4 flex items-center justify-between hover:bg-gray-900/70 transition">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            <label for="is_active" class="block font-bold text-white text-sm cursor-pointer select-none">Juez Activo</label>
                                            <p class="text-gray-500 text-xs mt-0.5">Habilitar o deshabilitar acceso al sistema</p>
                                        </div>
                                    </div>
                                    
                                    <label class="relative inline-flex items-center cursor-pointer">
                                        <input type="checkbox" id="is_active" name="is_active" value="1" {{ old('is_active', $judge->is_active) ? 'checked' : '' }} class="sr-only peer">
                            ...
                            <div class="px-8 py-5 bg-gray-900/50 border-t border-gray-700 flex items-center justify-end gap-4">
                                <a href="{{ route('judges.index') }}" class="px-4 py-2 text-sm font-bold text-gray-400 hover:text-white transition">
                                    Cancelar
                                </a>
